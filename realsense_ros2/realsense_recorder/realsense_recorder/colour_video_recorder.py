#!/usr/bin/env python3
"""H.265 video recorder for RealSense color/IR streams."""
...
class StreamRecorder:
    def __init__(self, topic: str, output_dir: str, crf: int,
                 encoder: str, fps: float, max_queue: int, logger):
        self.topic = topic
        self.logger = logger
        self.frame_idx = 0
        self._proc = None
        ...
        self._crf = crf
        if "_Color" in topic and encoder == "hevc_nvenc":
    ...
    def _start(self, width: int, height: int, pixel_fmt: str):
        ...
        output_fmt = "nv12" if self._encoder in {"hevc_nvenc", "h264_nvenc"} else "yuv420p"
...
class ColourVideoRecorder(Node):
    def __init__(self):
        ...
        self.declare_parameter("output_dir", os.path.expanduser("~/realsense_recordings"))
        ...
        compressed_topics = [str(t) for t in self.get_parameter("compressed_topics").value if t]
        override_entries = [str(t) for t in self.get_parameter("topic_fps_overrides").value if t]
        ...
        for topic in topics:
            ...
            recorder = StreamRecorder(topic, output_dir, crf, encoder, topic_fps, max_queue, self.get_logger())
        ...
        for topic in compressed_topics:
            topic_fps = fps_overrides.get(topic, fps)
            recorder = CompressedStreamRecorder(topic, output_dir, crf, encoder, topic_fps, max_queue, self.get_logger())
            self._recorders[topic] = recorder
            self._subscriptions.append(
                self.create_subscription(
                    CompressedImage,
